    ...
    public class Square : MonoBehaviour, ISquareItemCommon, IField, IMarmaladeTargetable
    {
        ...
        public Item Item
        {
            ...
            set
            {
                ...
                if(item != null)
                {
                    if (GetSubSquare().aboveItem && (item.GetSpriteRenderer().sortingOrder >= GetSpriteRenderer().sortingOrder || subSquares.Count>1 && subSquares[^2].mainRenderer.sortingOrder >= item.GetSpriteRenderer().sortingOrder))
                    {
                        item.GetComponent<IItemInterface>().SetOrder(GetDeepestSubSquareAboveItem().GetSpriteRenderer().sortingOrder );
                        var sortingOrder = item.GetSpriteRenderer().sortingOrder;
                        foreach (var subSquare in subSquares)
                        {
                            if(subSquare.aboveItem)
                            {
                                sortingOrder++;
                                subSquare.mainRenderer.sortingOrder = sortingOrder;
                            }
                        }
                    }
        ...
        public Item GenItem(bool falling = true, ItemsTypes itemType = ItemsTypes.NONE, int color = -1, ItemForEditor squareBlockItem=null,  bool noAnim = false)
        {
            ...
            {
                var targetComponent = item.AddCachedComponent<TargetComponent>();
                targetComponent.field = field;
            }
            ...
            if (LevelManager.THIS.DebugSettings.DestroyLog) DebugLogKeeper.Log(name + " gen item " + item.name + " color " + colorableComponent.color + " pos " + item.transform.position, DebugLogKeeper.LogType.Destroying);
...
//        if (!nextSq?.IsNone() ?? false)
        ...
        private void CreateSubblock(SquareTypes _type, GameObject prefab, int index, SquareTypeLayer layerSquare)
        {
            if (prefab != null && _type != SquareTypes.EmptySquare && _type != SquareTypes.NONE)
            {
                ...
                if(!layerSquare.anotherSquare)
                {
                    ...
                    b.GetComponent<SpriteRenderer>().sortingOrder = 0 + index + mainRenderer.sortingOrder;
        ...
        public List<Item> FindMatchesAround(int matches = 3, Hashtable countedSquaresGlobal = null)
        {
        ...
        {
            return (GetSubSquare()?.canMoveIn ?? false) && (!IsUndestroyable()) ;
        }
        /// <summary>
        /// Next move event
        /// </summary>
        ...
        public void DestroyBlock(bool destroyTarget=false)
        {
            ...
            if (subSquares.Count > 0)
            {
                ...
                if (subSquare.GetCachedComponent<TargetComponent>() == null)
                {
        ...
        {
            var enumerable = field.GetSquares().Where(i=> i.subSquares.Contains(this));
        ...
        /// <summary>
        /// Get deepest sub-square which option  "above the item" enabled
        /// </summary>
        /// <returns></returns>
        private Square GetDeepestSubSquareAboveItem()
        {
            for (var i = 0; i < subSquares.Count; i++)
            {
                if (subSquares[i].aboveItem)
                    return subSquares[i];
            }
            return null;
        }
        ...
        public Sprite GetSprite()
        {
            ...
            if (subSquares.Count > 0)
                return subSquares.Last().mainRenderer.sprite;
        ...
        public SpriteRenderer[] GetSpriteRenderers()
        {
            ...
            sq = subSquares.Count > 0 ? subSquares.WhereNotNull().Select(i => i.mainRenderer) : new[] {mainRenderer};
            return sq.ToArray();
        }
        ...
        public SpriteRenderer GetSpriteRenderer()
        {
            if (subSquares.LastOrDefault() != null && subSquares.Count > 0)
                return subSquares.LastOrDefault()?.mainRenderer;
            return mainRenderer;
        }
