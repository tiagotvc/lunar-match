...
// #endif
        ...
        {
            // Avoid LINQ's Any() which causes allocations
            if (sq == null) return null;
            
            // Check restrictions manually to avoid LINQ allocations
            for (int i = 0; i < sq.directionRestriction.Count; i++)
            {
                if (sq.directionRestriction[i].x == vector2.x && 
                    sq.directionRestriction[i].y == vector2.y)
                    return null;
            }
            
            // Calculate new position directly without creating intermediate Vector2
            int newCol = sq.col + (int)vector2.x;
            int newRow = sq.row + (int)vector2.y;
            
            // Get square directly using coordinates rather than creating a new position
            return sq.field.GetSquare(newCol, newRow);
        }
