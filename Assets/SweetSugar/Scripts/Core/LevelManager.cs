...
//game state enum
    ...
    // References to replace transform.Find calls
    /// <summary>
    /// core-game class, using for handle game states, blocking, sync animations and search mathing and map
    /// </summary>
    public class LevelManager : MonoBehaviour
    {
        // References to replace GameObject.Find calls
        [SerializeField] private Canvas canvasBack;
        [SerializeField] private Transform canvasScoreTransform;

        [SerializeField] private GameObject levelCanvasObject;
        [SerializeField] private GraphicRaycaster levelCanvasRaycaster;

        // MenuReference cached items
        private GameObject preFailedMenu;
        private GameObject preCompleteBanner;
            ...
            set
            {
                ...
                switch (value)
                {
                    ...
                    case GameState.WaitForPopup://waiting for pre game banners
                        ...
                        if (canvasBack == null)
                            canvasBack = ReferenceRestorer.FindCanvasBack();
                        if (canvasBack != null) canvasBack.GetComponent<GraphicRaycaster>().enabled = false;
                    ...
                    case GameState.PreFailed://chance to continue the game, shows menu PreFailed
                        ...
                        {
                            var preFailedObject = ReferenceRestorer.FindPreFailed();
                            if (preFailedObject != null)
                            {
                                preFailedObject.GetComponent<PreFailed>().SetFailed();
                                preFailedObject.SetActive(true);
                            }
                        });
                        break;
                    case GameState.BombFailed:
                        LeanTween.Framework.LeanTween.delayedCall(0.3f, () =>
                        {
                            ...
                            if (preFailedObject != null)
                            {
                                preFailedObject.GetComponent<PreFailed>().SetBombFailed();
                                preFailedObject.SetActive(true);
                            }
                        });
                        break;
        ...
        public void EnableMap(bool enable)
        {
            ...
            else
            {
                ...
                // Use cached references with fallbacks
                if (levelCanvasObject == null && Level != null)
                    levelCanvasObject = ReferenceRestorer.FindLevelCanvasObject(Level);
                if (levelCanvasRaycaster == null && levelCanvasObject != null)
                    levelCanvasRaycaster = ReferenceRestorer.FindLevelCanvasRaycaster(levelCanvasObject);
                    
                if (levelCanvasRaycaster != null)
                {
                    levelCanvasRaycaster.enabled = false;
                    levelCanvasRaycaster.enabled = true;
                }
                else if (Level != null)
                {
                    ReferenceRestorer.ToggleLevelCanvasRaycaster(Level);
                }
            }
        ...
        private void Awake()
        {
            ...
            if (canvasBack == null)
                canvasBack = ReferenceRestorer.FindCanvasBack();
            if (canvasScoreTransform == null)
                canvasScoreTransform = ReferenceRestorer.FindCanvasScoreTransform();
                
            if (Level != null)
            {
                if (levelCanvasObject == null)
                    levelCanvasObject = ReferenceRestorer.FindLevelCanvasObject(Level);
                if (levelCanvasRaycaster == null && levelCanvasObject != null)
                    levelCanvasRaycaster = ReferenceRestorer.FindLevelCanvasRaycaster(levelCanvasObject);
            }
        ...
        {
            // Early exit without allocations
            if (animateItems.Count > 0 || findMatchesStarted) return; 
            
            // Get items once to avoid multiple calls
            var items = field.GetItems();
            
            // Check falling ingredients without LINQ
            bool hasAnyFallingIngredients = false;
            for (int i = 0; i < items.Count; i++)
            {
                var item = items[i];
                if (item != null && 
                    item.currentType == ItemsTypes.INGREDIENT && 
                    (item.falling || item.transform.position.y > item.square.transform.position.y + 0.1f))
                {
                    hasAnyFallingIngredients = true;
                    break;
                }
            }
            
            if (hasAnyFallingIngredients)
            {
                waitForIngredients = true;
                StartCoroutine(WaitForIngredientsToSettle());
                return;
            }
            
            if (waitForIngredients) return;
            ...
            {
                // Check for expired bomb timers without LINQ
                bool hasExpiredBomb = false;
                for (int i = 0; i < items.Count; i++)
                {
                    var item = items[i];
                    if (item == null) continue;
                    
                    var timeBomb = item.GetCachedComponent<itemTimeBomb>();
                    if (timeBomb != null && timeBomb.timer <= 0)
                    {
                        hasExpiredBomb = true;
                        break;
                    }
                ...
                if (hasExpiredBomb)
                {
                    gameStatus = GameState.BombFailed;
                }
            }
        ...
        private IEnumerator PreWinLoop(int limit)
        {
            ...
            preCompleteBanner = ReferenceRestorer.FindPreCompleteBanner();
            if (preCompleteBanner != null)
            {
        ...
        private void Update()
        {
            ...
            if (Input.GetKeyUp(DebugSettings.Back) && DebugSettings.enableHotkeys)
            {
                ...
                {
                    if (THIS.gameStatus == GameState.Map)
                        Application.Quit();
                }
            }
        ...
        private bool waitForIngredients;
        private float ingredientCheckDelay = 1f;
        private bool turnEndTriggered;
        private int lastTurnEndMoveID = -1;

        private IEnumerator FallingDown()
        {
            OnMove?.Invoke();
            findMatchesStarted = true;
            turnEndTriggered = false;  // Reset the flag at start of new move
            ...
            if (gameStatus == GameState.Playing)
            {
                if (!animateItems.Any())
                {
                    if (lastTurnEndMoveID != moveID)
                    {
                        lastTurnEndMoveID = moveID;
                        OnTurnEnd?.Invoke();
                    }
                    THIS.CheckWinLose();
                }
                ...
                {
                    yield return new WaitWhile(()=>animateItems.Any() || checkTarget || waitForIngredients);
                    if (lastTurnEndMoveID != moveID)
                    {
                        lastTurnEndMoveID = moveID;
                        OnTurnEnd?.Invoke();
        ...
        public List<Square> GetRowSquare(int row)
        {
            ...
            for (var col = 0; col < levelData.maxCols; col++)
            {
                ...
                if(square.type == SquareTypes.UndestrBlock && AdditionalSettings.StripedStopByUndestroyable) break;
                if (!square.IsNone()) 
        ...
        public List<Item> GetRow(Square square)
        {
            ...
            for (var col = square.col; col < levelData.maxCols; col++)
            {
                ...
                if(square1.type == SquareTypes.UndestrBlock && AdditionalSettings.StripedStopByUndestroyable) break;
        ...
        public void ShowPopupScore(int value, Vector3 pos, int color)
        {
            if (showPopupScores)
            {
                ...
                poptxt.transform.SetParent(canvasScoreTransform);
        ...
        private IEnumerator WaitForIngredientsToSettle()
        {
            yield return new WaitForSeconds(ingredientCheckDelay);
            
            // Get items once to avoid multiple calls
            var items = field.GetItems();
            
            // Check if any ingredients are still falling without LINQ
            ...
            if (!anyStillFalling)
            {
                waitForIngredients = false;
                CheckWinLose(); // Recheck win/lose conditions after ingredients settle
            }
            else
            {
                StartCoroutine(WaitForIngredientsToSettle()); // Keep waiting if still falling
            }
        }
