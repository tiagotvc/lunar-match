...
namespace SweetSugar.Scripts.Items
{
    ...
    public class Item : ItemMonoBehaviour, ISquareItemCommon, IField, IColorChangable, IMarmaladeTargetable
    {
        ...
        public static Item usedItemSecond;
        ...
        protected virtual void OnEnable()
        {
            ...
            InputHandler.OnMove += OnMouseMove;
            
            // Periodically check type changes and squares below without Update
        ...
        // Replace Update with coroutine for less frequent checks
        private IEnumerator PeriodicChecks()
        {
            while (gameObject.activeSelf)
            {
                ...
                // Check square below only when needed
                if (!falling && square && LevelManager.GetGameStatus() != GameState.RegenLevel && 
                    field == LevelManager.THIS.field && usedItem != this && usedItemSecond != this && 
        ...
        public void OnMouseMove(Vector2 mousePosition)
        {
            if (dragThis && !LevelManager.THIS.findMatchesStarted)
            {
                ...
                {
                    (usedItem, usedItemSecond) = SwitchDirection(deltaPos);
                }
            }
        }
        
        // Called externally when dragging starts
        public void StartDragging()
        {
            dragThis = true;
        }
        
        // Called externally when dragging ends
        public void StopDragging()
        {
            dragThis = false;
            usedItem = usedItemSecond = null;
        }
        
        // Used to manually trigger type changes
        ...
        // Method to explicitly trigger square below check
        public void TriggerSquareBelowCheck()
        {
            if (!falling && square && LevelManager.GetGameStatus() != GameState.RegenLevel && 
                field == LevelManager.THIS.field && usedItem != this && usedItemSecond != this && 
                NextType == ItemsTypes.NONE)
        ...
        public virtual void InitItem()
        {
            ...
            bool animoffset=false || _spriteObjectRef.transform.localPosition != Vector3.zero;
        ...
        private void Awake()
        {
            ...
            _spriteObjectRef = transform.Find("Sprite");
        }
        ...
        private IEnumerator RareUpdate()
        {
            while (true)
            {

                if (dontDestroyForThisCombine)
                {
        ...
        public (Item currentItem, Item neighborItem) SwitchDirection(Vector3 delta)
        {
            deltaPos = delta;
        ...
        private IEnumerator Switching()
        {
            if (switchDirection != Vector3.zero && neighborSquare)
            {
                ...
                // condition for determining invalid moves
                ...
                bool invalidItemCombination = NotContainsBoth(ItemsTypes.MULTICOLOR, ItemsTypes.MULTICOLOR) ||
                                             ContainsBoth(ItemsTypes.MULTICOLOR, ItemsTypes.INGREDIENT) ||
        ...
        private Transform _spriteObjectRef;
            ...
            if (!needFall)
            {
                ...
                if(!square.GetAllNeghborsCross().Any(i => (i.Item && i.Item.falling && i.Item.color == color)))
                {
                    ...
                    foreach (var combine in combines2)
                    {
                        if (combine.nextType == ItemsTypes.NONE)
                            continue;
                        var firstOrDefault = combine.items[0];
                        if (firstOrDefault != null && firstOrDefault.NextType == ItemsTypes.NONE)
                        {
                            firstOrDefault.NextType = combine.nextType;
                        }
                    }
        ...
        public void CheckNearEmptySquares()
        {
            ...
            checkingSquare = field.GetSquare((Vector3)square.GetPosition() + Quaternion.Euler(0f, 0f, 90f) * lookingDirection1);
            if (checkingSquare && (!checkingSquare.IsItemAbove() && GetBarrierBefore(checkingSquare)))
                nearEmptySquareDetected = CheckNearSquare(nearEmptySquareDetected, checkingSquare);
        }
        ...
        public void DestroyItem(bool showScore = false, bool particles = true, Item explodedItem = null, bool explEffect = false)
        {
            ...
            if (!explodedItem && (!marmaladeTarget || (!marmaladeTarget?.activeSelf ?? false)) && (combinationID == explodedItem?.GetHashCode() || dontDestroyOnThisMove || needFall) || !Explodable)
            {
                StopDestroy();
                return;
            }
        ...
        private IEnumerator DestroyCor(bool showScore = false, bool particles = true, Item explodedItem = null, bool explEffect = false)
        {
            ...
            SoundBase.Instance?.PlayOneShot(SoundBase.Instance?.destroy[Random.Range(0, SoundBase.Instance.destroy.Length)]);
