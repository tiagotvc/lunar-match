    ...
    public class LevelData
    {
        ...
        public void SetItemTarget(Item item)
        {
            foreach (var _subTarget in TargetCounters)
            {
                ...
                {
                    var targetComponent = item.gameObject.AddCachedComponent<TargetComponent>();
                    targetComponent.field = item.field;
                }
            }
        }
        ...
        {
            {
                ...
                {
                    if (subTargetContainer.targetLevel.setCount == SetCount.FromLevel)
                        subTargetContainer.changeCount(1);
                    var targetComponent = gameObject.AddCachedComponent<TargetComponent>();
                    targetComponent.field = gameObject.GetComponent<Square>().field;
                }
            }
        }
        ...
        {
            for (int i = 0; i < TargetCounters.Count; i++)
            {
                if (!TargetCounters[i].IsTotalTargetReached())
                    return false;
            }
            return true;
        }
        ...
        public bool IsTargetReachedSublevel()
        {
            for (int i = 0; i < TargetCounters.Count; i++)
            {
                if (!TargetCounters[i].IsTargetReachedSublevel())
                    return false;
            }
            return true;
        }
        ...
        public bool WaitForMoveOut()
        {
            for (int i = 0; i < TargetCounters.Count; i++)
            {
                if (TargetCounters[i].NotFinishUntilMoveOut)
                    return true;
            }
            return false;
        }
    ...
    public class ItemForEditor
    {
        ...
        { 
            get
            {
                ...
                {
                    var sprites = colors.GetSprites(LevelData.THIS.levelNum);
                    if (sprites != null && Color < sprites.Count())
                    {
                        Texture = sprites.ElementAt(Color).texture;
                        if (Texture != null)
        ...
        public void SetColor(int color, int currentLevel)
        {
            ...
            if(colors && colors.GetSprites(currentLevel).Count() > color)
                Tex = colors.GetSprites(currentLevel)[color].texture;
        }
    }
}